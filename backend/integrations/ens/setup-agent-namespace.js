...
async function registerRootName(label, ownerAddress) {
  ...
  if (!isAvailable) {
    ...
    // 3. Approve the registrar to spend the payment token (with headroom in
    //    case the oracle price moves between quote and reveal). If an
    //    allowance already covers the quote, reuse it — approvals are
    //    idempotent, so sending a fresh one is harmless but costs gas.
    const totalCost = base + premium;
    const approveAmount = (totalCost * 12n) / 10n; // totalCost * 1.2
    console.log(`Approving ${approveAmount} of payment token ${paymentToken} for registrar...`);
    ...
    if (allowance < totalCost) {
      const approveTx = await paymentTokenContract.approve(ETH_REGISTRAR_ADDRESS, approveAmount);
      console.log(`Approval transaction sent. tx: ${approveTx.hash}`);
      await approveTx.wait();
      console.log(`Approved ${approveAmount} of payment token ${paymentToken} for registrar. tx: ${approveTx.hash}`);
    } else {
      console.log(`Existing allowance ${allowance} already covers the registration cost — reusing it.`);
    }

    // Re-check allowance to ensure approval succeeded
    const allowanceAfter = await paymentTokenContract.allowance(wallet.address, ETH_REGISTRAR_ADDRESS);
    if (allowanceAfter < totalCost) {
      throw new Error(
        `Approval failed: allowance ${allowanceAfter} is less than required ${totalCost}. Check your payment token balance and approval.`
      );
    }
    console.log(`Approval confirmed: allowance is ${allowanceAfter}. Proceeding with registration.`);
...
async function deployAgentSubnameRegistrar(registryAddress, minDurationSeconds) {
  ...
  // default `out/` layout.
  // Path relative to this file (backend/integrations/ens/) up to the repo
  // root contracts-ens/ foundry output.
