 ...
 *
 * Covers:
 *   - tools/list reports the expected 3 tools
...
async function main() {
  ...
  try {
    ...
    assert(JSON.stringify(names) === JSON.stringify(["get_agent_report", "get_factoring_rate", "list_agents"]), `unexpected tools: ${names}`);
    ...
    console.log("SMOKE OK — 3 tools + resource verified against the live server protocol.");
  } finally {
    await client.close();
    mockServer.close();
  }
